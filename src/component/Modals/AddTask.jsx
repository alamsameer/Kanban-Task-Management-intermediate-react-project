...
function Addtask({ closeModal }) {
  ...
  const [inputs, setInputs] = useState({status:"To Do",subtasks:new Array(2).fill({title:"",isCompleted:false})})
  ...
  const removeSubTask = (index) => {
    ...
    setInputs((values)=>({...values,columns:newSubtask}))
  }
...
function SubTask({ subtask, i,handleInputs,inputs, removeSubTask }) {
  ...
  return <>
    <label htmlFor={"subtask" + i}>
      <input type="text" name="subtask" id="subtask" className={theme} value={inputs.subtask} onChange={(e) => { handleInputs(e, i) }} placeholder="Add subtask" />
    </label>
